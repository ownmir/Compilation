...
class FieldTypeTests(TestCase):
    def test_field_widjet_type(self):
        form = ExampleForm()
...
class InputClassTests(TestCase):
    def test_unbound_field_inition_state(self):
        form = ExampleForm()  # unbound form
        self.assertEquals('form-control ', input_class(form['name']))

    def test_vakid_bound_field(self):
        form = ExampleForm({'name': 'neo', 'password': '123'})  # bound form (field+data)
